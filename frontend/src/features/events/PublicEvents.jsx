...
export default function PublicEvents() {
 ...
 <p className="text-slate-600 font-medium">Temukan event photobooth dan ambil foto</p>
 </div>
 </div>
 ...
 {isLoading ? (
 <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">{[1, 2, 3].map((i) => <div key={i} className="bg-white rounded-xl border border-border-subtle h-64 animate-pulse" />)}</div>
 ) : (
 ...
 <Link to={`/events/${event.slug}`} className="block bg-white rounded-xl border border-border-subtle shadow-card hover:shadow-card transition-all overflow-hidden group">
 ...
 <span className={`text-xs px-2.5 py-1 rounded-lg font-bold border border-border-subtle ${statusBadge[event.status] ?? 'bg-slate-200 text-slate-700'}`}>{statusLabel[event.status] ?? event.status}</span>
 </div>
 </div>
 <div className="p-5">
 <h3 className="font-bold text-charcoal mb-1">{event.name}</h3>
 <div className="flex items-center gap-3 text-xs text-warm-gray font-medium mb-4">
